...
:root{
  ...
  --primary:#FF8A4C; --primarySoft:#FFE3D3; --accent:#E36C2D;
  --card:#FFFFFF; --border:#F3D7C8; --shadow:0 12px 30px rgba(0,0,0,.08);
}
.login{ min-height:100vh; display:grid; place-items:center; padding:2rem; background:var(--bg); }
.login__card{ width:100%; max-width:26rem; background:var(--card); border:1px solid var(--border);
  border-radius:1rem; padding:2rem; box-shadow:var(--shadow); }
.login__title{ font-size:1.875rem; font-weight:800; color:var(--text); text-align:center; margin-bottom:1.25rem; }
.login__form{ display:flex; flex-direction:column; gap:.75rem; }
.login__label{ font-size:.9rem; color:var(--text); font-weight:600; }
.login__input{ width:100%; border:1px solid #e7e2de; background:#fff; color:var(--text);
  border-radius:.75rem; padding:.9rem 1rem; outline:none; transition:border .15s, box-shadow .15s; }
.login__input:focus{ border-color:var(--primary); box-shadow:0 0 0 3px rgba(255,138,76,.25); }
.login__pwdwrap{ position:relative; }
.login__toggle{ position:absolute; right:.6rem; top:50%; transform:translateY(-50%);
  padding:.35rem .6rem; border-radius:.5rem; border:1px solid var(--border);
  background:var(--primarySoft); color:#8a4d2b; font-size:.8rem; font-weight:600; cursor:pointer; }
...
.login__error{ margin-top:.5rem; background:#ffe6e6; border:1px solid #ffc7c7; color:#8a1f1f;
  padding:.75rem 1rem; border-radius:.75rem; font-size:.9rem; }
.btn{ margin-top:1rem; width:100%; padding:.9rem 1rem; border-radius:.9rem; font-weight:700; cursor:pointer; border:none; }
...
.btn--google{ background:#fff; color:#1f1f1f; border:1px solid #e8e8e8; display:flex; align-items:center;
  justify-content:center; gap:.6rem; padding:.9rem; border-radius:1rem; }
